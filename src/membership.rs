...
impl Membership {
    ...
    pub fn solo(node_id: NodeId) -> Self {
        ...
        res.nodes.insert(node_id, MembershipType::LEARNER);
        res
    }
    pub fn bootstrap(
    ...
    pub fn begin_change(&mut self, new_members: BTreeSet<NodeId>) {
        for (k, v) in &mut self.nodes {
            v.begin_change(new_members.contains(k));
        }
    }
    pub fn set_learners(&mut self, learners: BTreeSet<NodeId>) {
        self.nodes
